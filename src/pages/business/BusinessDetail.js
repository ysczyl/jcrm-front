...
import styles from './BusinessDetail.less';
...
class BusinessDetail extends Component {
  state = {
    ...
    detail:this.props.location.state.record,
  };
  ...
  render() {
    ...
    const description = (
      <DescriptionList className={styles.headerList} size="small" col="2">
        <Description term="所有人">{ this.state.detail.username }</Description>
        <Description term="创建时间">{moment(this.state.detail.ctime).format('YYYY-MM-DD HH:mm:ss')}</Description>
        <Description term="相关描述">
        {this.state.detail.description}
        </Description>
        <Description term="下一步">{ this.state.detail.nextStep }</Description>
      </DescriptionList>
    );
    ...
    const extra = (
      <Row>
        <Col xs={24} sm={6}>
          <div className={styles.textSecondary}>可能性</div>
          <div className={styles.heading} >{this.state.detail.possibility}%</div>
        </Col>
      </Row>
    );
    ...
    return (
      <PageHeaderWrapper
        title={<span>单号：{this.state.detail.oppName }</span>}
        logo={
